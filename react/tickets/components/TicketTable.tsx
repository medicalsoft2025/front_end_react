...
export const TicketTable = () => {
    ...
    useEffect(() => {
        const fetchReasons = async () => {
            try {
                ...
                const reasonsMap = response.reasons.reduce(
                    (acc: any, reason: any) => {
        ...
        channel.bind(
            ...
            function (data: { ticket: TicketDto | any } | any) {
                ...
                const newTicketData: TicketTableItemDto | any = {
                    ...
                    priority:
                        ticketPriorities[data.ticket.priority] || "NORMAL",
        ...
        channel.bind("ticket.state.updated", function (data) {
            setData((prevData) => {
                ...
                if (index > -1) {
                    ...
                    newData[index].statusColor =
                        ticketStatusColors[data.newState];
    ...
    useEffect(() => {
        if (
            !data.length ||
            !loggedUser ||
            !Array.isArray(loggedUser.availabilities)
        ) {
            setFilteredData([]);
            return;
        }
        ...
        const filteredAndSorted = data
            .filter((item) => {
                ...
                const statusMatch =
                    item.status === "PENDING" || item.status === "CALLED";

                const reasonMatch = loggedUser.availabilities.some(
                    (availability) => {
                        const allowed =
                            availability?.module?.allowed_reasons || [];
                        return (
                            item.reason_key && allowed.includes(item.reason_key)
                        );
                    }
                );
    ...
    const callTicket = async (ticket: any) => {
        ...
        const hasActiveTicket = data.some(
            (t) =>
                t.status === "CALLED" && t.module_id === user?.today_module_id
        );
    ...
    const slots = {
        3: (cell, data: TicketTableItemDto) => (
            <span
                className={`badge badge-phoenix badge-phoenix-${
                    ticketStatusColors[data.status] || "secondary"
                }`}
            >
        ...
        4: (cell, data: TicketTableItemDto) => {
            ...
            const firstPendingIndex = filteredData.findIndex(
                (t) => t.status === "PENDING"
            );
            const hasActiveTicket = filteredData.some(
                (t) =>
                    t.status === "CALLED" &&
                    t.module_id === loggedUser?.today_module_id
            );
