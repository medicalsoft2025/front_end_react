    ...
    fieldConfigs.current.forEach((fieldConfig, fieldPath) => {
      if (fieldConfig.rules) {
        ...
        const relevantRules = fieldConfig.rules.filter(rule => {
          ...
          return actions.some(action => action.expression && action.expression.includes(`{{${changedFieldPath}}}`));
        });
            ...
            switch (action.type) {
              ...
              case "hide":
                updates[action.target] = {
                  ...updates[action.target],
                  visible: action.type === "show" ? conditionResult : !conditionResult
                };
                break;
              case "enable":
              case "disable":
                updates[action.target] = {
                  ...updates[action.target],
                  disabled: action.type === "disable" ? conditionResult : !conditionResult
                };
                break;
              case "setValue":
                if (conditionResult && action.target) {
                  ...
                  setValue(action.target, newValue, {
                    shouldValidate: true,
                    shouldDirty: false,
                    shouldTouch: false
                  });
