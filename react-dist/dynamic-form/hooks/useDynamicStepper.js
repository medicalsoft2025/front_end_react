...
export function useDynamicStepper({
  ...
  const validStep = () => {
    ...
    const validateContainer = (container, currentPath = getActualFormGroupPath()) => {
      let containerValid = true;

      // Actualizar path si es un form anidado
      let newPath = currentPath;
      if (container.type === "form" && container.name) {
        newPath = currentPath ? `${currentPath}.${container.name}` : container.name;
      }

      // Validar campos requeridos
      container.fields?.forEach(field => {
        if (field.required || field.validation?.required) {
          const fieldPath = newPath ? `${newPath}.${field.name}` : field.name;
          const fieldError = getValueByPath(form.formState.errors, fieldPath);
      ...
      container.containers?.forEach(childContainer => {
        containerValid = containerValid && validateContainer(childContainer, newPath);
      });
