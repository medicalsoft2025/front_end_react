                                ...
                                <div class="card mt-4" id="infoEntidad" style="display: none;">
                                    <form class="" id="formEntityInvoice" data-wizard-form="">
                                        <div class=" card-body">
                                            ...
                                            <div id="koneksiPatientContainer" class="d-none">
                                                <div class="row mb-3">
                                                    <div class="col-12">
                                                        <div class="mb-3">
                                                            <label for="formSlugs" class="form-label">Autorizar por</label>
                                                            <select id="formSlugs" class="form-control">
                                                                <option value="">Seleccione una opción</option>
                                                            </select>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="koneksiPatientId" class="form-label">ID</label>
                                                            <input type="text" class="form-control" id="koneksiPatientId">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="d-flex justify-content-center mb-5">
                                                    <button id="btnAuthorizePatient" type="button" class="btn btn-primary">Autorizar paciente</button>
                                                </div>
                                            </div>
                                            <div id="koneksiPatientLoadingContainer" class="d-none text-center my-3">
                                                <div class="spinner-border text-primary" role="status">
                                                    <span class="visually-hidden">Loading...</span>
                                                </div>
                                                <p class="mt-2" id="koneksiPatientLoadingLabel"></p>
                                            </div>
                                            <div id="koneksiError" class="d-none">
                                                <div class="alert alert-danger d-flex align-items-center" role="alert">
                                                    <div id="koneksiErrorMessage"></div>
                                                </div>
                                            </div>
                                            <div id="koneksiSuccess" class="d-none">
                                                <div class="alert alert-success d-flex align-items-center" role="alert">
                                                    <div id="koneksiSuccessMessage"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
                                                                ...
                                                                <div class="card-body">
                                                                    ...
                                                                    <div class="row g-3">
                                                                        ...
                                                                        <div class="col-sm-3 col-md-6 col-lg-12">
                                                                            ...
                                                                            <label class="form-label" for="admissionFormDocument">Documento</label>
                                                                            <input class="form-control" id="admissionFormDocument" type="file">
...
<script type="module" src="./Citas/assets/js/admissionFormKoneksi.js"></script>
<script type="module">
    import {
        admissionService,
        productService,
    ...
    import {
        init as initKoneksi
    } from "./Citas/assets/js/admissionFormKoneksi.js";
    ...
    function populateProductSelect(products) {
        ...
        if (Array.isArray(products)) {
            products.forEach(product => {
                ...
                if (attributes && attributes.name && product?.relationships?.exam_type?.type ==
                    "LABORATORY") {
                    const option = document.createElement('option');
    ...
    async function addProductToTable(product, admission, isEntityActive) {

        const productsByExamRecipe = [];

        if(admission.exam_recipe_id !== null){
            productsByExamRecipe = await productService.getProductsByExamRecipe(admission.exam_recipe_id);
        }
        ...
        `;
                ...
                if (productsByExamRecipe.length) {
                    productsByExamRecipe.forEach(element => {
                        const productRowInArray = document.createElement('tr');

                        let validationPriceProductInArray = entitySwitch.checked ? element.copayment : element.sale_price;
                        const priceProductInArray = new Intl.NumberFormat('es-CO').format(validationPriceProductInArray);
                        const totalPriceInArray = new Intl.NumberFormat('es-CO').format(validationPriceProductInArray);
                        productRowInArray.innerHTML = `
            <td class="small">${element.id}</td>
            <td class="small">${element.name || 'Producto sin nombre'}</td>
            <td class="small">${ entitySwitch.checked ? element.copayment : element.sale_price || 'N/A'}</td>
            <td class="small">1</td>
            <td class="small">${element.taxes || '0'}</td>
            <td class="small">${ entitySwitch.checked ? totalPriceInArray : totalPriceInArray || 'N/A'}</td>
            <td class="small"><i class="far fa-trash-alt text-danger" onclick="removeRow(this)"></i></td>
            <td></td>`;

                        tableBody.appendChild(productRowInArray);
                    });
    ...
    async function guardarAdmision() {
        ...
        const requestData = {
            ...
            admission: {
                ...
                koneksi_claim_id: localStorage.getItem(`consultation_claim_id_${globalAdmission.patient_id}_${globalAdmission.id}`) || null,
            },
            invoice: {
        ...
        await admissionService.createAdmission(requestData, globalAdmission.patient_id)
            .then(async response => {
                if (response) {
                    ...
                    const admissionId = response.admission_data.id;
                    const minioId = await guardarDocumentoAdmision('admissionFormDocument', admissionId);

                    if (minioId) {
                        await admissionService.update(admissionId, {
