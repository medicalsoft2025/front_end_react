...
document.getElementById('btnGuardarIncapacidad').addEventListener('click', function () {
    ...
    if (data.accionModalCrearIncapacidad === 'crear') {
        patientDisabilityService.createForParent(patientId, data)
            .then(() => handleSuccess(AlertManager, 'Se ha creado el registro exitosamente'))
            .catch((err) => handleError(AlertManager, err));
    } else {
        patientDisabilityService.update(data.id, data)
            .then(() => handleSuccess(AlertManager, 'Se ha actualizado el registro exitosamente'))
            .catch((err) => handleError(AlertManager, err));
    }
});
