...
export const AppointmentsFinishedTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    filtered = filtered.filter(
      (appointment) => `${appointment.stateKey}` === "consultation_completed"
    );
  ...
  const slots = {
    0: (cell, data: AppointmentTableItem) => (
      <a href={`postConsultationGestion?patientId=${data.patientId}`}>{data.patientName}</a>
    ),
      ...
      <div className="card">
        ...
        >
          ...
          >
            <thead>
              <tr>
                <th className="border-top custom-th text-start" style={{ width: "200px" }}>Nombre</th>
