...
// URL de la API
const apiUrl = 'https://dev.monaros.co/api/v1/admin/products';
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            productData = data.data.map(product => ({
                ...
                category: "Categoria desconocida",  // Aquí podrías mapear según el product_type_id
                price: parseFloat(product.attributes.sale_price) || 0.00,  // Si el precio de venta está disponible
                image: "https://via.placeholder.com/80",  // Placeholder ya que no hay imagen en la API
            }));
            renderProducts();  // Una vez obtenidos los productos, renderizarlos
        })
        .catch(error => {
