...
document.getElementById("finishStep").addEventListener("click", function () {
  ...
  if (!id) {
    patientService
      .storePatient(data)
      .then(async (res) => {
        console.log('Res create patient', res);

        AlertManager.success({
          text: "Se ha creado el registro exitosamente.",
        });

        const minioId = await guardarArchivoPaciente('uploadImage', res.patient_id);
    ...
    patientService
      ...
      .then(async () => {
        ...
        const minioId = await guardarArchivoPaciente('uploadImage', id);

        console.log('MinioId', minioId);
...
document
  ...
  .addEventListener("click", function () {
    ...
    companionsTemp.push({
      ...
      is_active:
        companionData.is_active === "true" || companionData.is_active === true, // Asegurar booleano
