...
export const Commissions = () => {
  ...
  const formatDataToTreeNodes = users => {
    const nodes = users.map((user, userIndex) => {
      ...
      const montoEntidad = user.admissions?.reduce((sum, admission) => {
      ...
      let baseCalculo = 0;
      let comision = 0;
      const comisionesValidas = user.commissions?.filter(commission => commission.attention_type === "entity" && commission.percentage_base === "entity") || [];
      if (comisionesValidas.length > 0) {
        const comisionConfig = comisionesValidas[0];
        const serviciosComision = comisionConfig.services?.map(s => s.service_id) || [];
        baseCalculo = user.admissions?.reduce((sum, admission) => {
          ...
          if (productId && serviciosComision.includes(productId)) {
            ...
            const percentage = parseFloat(comisionConfig.percentage_value) || 0;
            return sum + amount * percentage / 100;
          }
          return sum;
        }, 0) || 0;
        const commissionValue = parseFloat(comisionConfig.commission_value) || 0;
        comision = baseCalculo * commissionValue / 100;
      }
      const entidadesFacturadas = user.invoices?.filter(invoice => invoice.type === "entity").length || 0;

      // Crear nodos hijos (admisiones)
      const children = user.admissions?.map((admission, admissionIndex) => {
        const servicioNombre = admission.appointment?.product?.attributes?.name || "Servicio no especificado";
        const montoAdmision = parseFloat(admission.entity_authorized_amount) || 0;
        const dateAdmission = new Date(admission.created_at).toISOString().split("T")[0];
  ...
  const profesionalTemplate = node => {
    return node.data.isLeaf ? /*#__PURE__*/React.createElement("span", {
      style: {
        paddingLeft: "30px"
      }
  ...
  const dateTemplate = node => {
    // console.log(node);
    return /*#__PURE__*/React.createElement("span", null, node.data.totalServices);
  };
