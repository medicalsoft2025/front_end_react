...
let usersMap = {};
let user = {};
let patient_id = new URLSearchParams(window.location.search).get("patient_id") || new URLSearchParams(window.location.search).get("patientId") || new URLSearchParams(window.location.search).get("id");
...
export async function generarFormatoIncapacidad(
  ...
  const tablePatient = await generarTablaPaciente(patientData, {
